from Live.Base import Vector, StringVector
from Live.Track import RoutingType, RoutingChannel, RoutingTypeVector, RoutingChannelVector
            ...
            while True:
                ...
                try:
                    ...
                    if message.address in self._callbacks:
                        ...
                        self.logger.info("MusaDSL4LiveOSC: Unknown OSC address: %s" % message.address)
                except ParseError:
                    self.logger.info("MusaDSL4LiveOSC: OSC parse error: %s" % (traceback.format_exc()))

        except socket.error as e:
            if e.errno == errno.EAGAIN:
                return
            else:
                self.logger.info("MusaDSL4LiveOSC: Socket error: %s" % (traceback.format_exc()))

        except Exception as e:
            self.logger.info("MusaDSL4LiveOSC: Error handling message: %s" % (traceback.format_exc()))
