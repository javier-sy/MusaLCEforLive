...
To install the script:
 ...
 - Clone this repo, or download/unzip and rename MusaDSL4LiveOSC-master to MusaDSL4LiveOSC
 - Move the MusaDSL4LiveOSC folder to the MIDI Remote Scripts folder inside the Ableton application: `/Applications/Ableton Live*.app/Contents/App-Resources/MIDI Remote Scripts`
 - Restart Live
 - In `Preferences > MIDI`, add the new MusaDSL4LiveOSC Control Surface that should appear. Live should display a message saying "MusaDSL4LiveOSC: Listening for OSC on port 11000"
 - On macOS, an activity log will be created at `/tmp/musa4liveosc.log` 

# Usage

MusaDSL4LiveOSC listens for OSC messages on port **11000**, and sends replies on port **11001**. 
...
| :------ | :-------------- | :---------- |
| /live/startup | | Sent to the client application when MusaDSL4LiveOSC is started |
...
| /live/track/get/available_input_routing_types | track_id | [type category id, type name, type category id, type name, ...] | Query all input routing types available to the track |
| /live/track/get/available_input_routing_channels | track_id | [channel category id, channel name, channel category id, channel name, ...] | Query all input routing channels available to the track |

CONTINUE:

| /live/track/get/input_routings | track_id | [routing type, ...] | Query all input routing types available to the track |
